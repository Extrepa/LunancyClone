 ...
 */
export function getTransformHandles(shape) {
 ...
 */
export function getHandleAt(screenX, screenY, shape, state) {
 ...
 */
export function drawSelection(ctx, shape, state, currentTool) {
  ctx.save();
 ...
 */
export function applyTransform(shape, handleType, x, y, transformStartShape) {
  const dx = x - transformStartShape.x - transformStartShape.width / 2;
  const dy = y - transformStartShape.y - transformStartShape.height / 2;
  const startX = transformStartShape.x;
  const startY = transformStartShape.y;
  const startW = transformStartShape.width;
  const startH = transformStartShape.height;
 ...
 */
export function findShapeAt(x, y, shapes) {
